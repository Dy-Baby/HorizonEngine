...
public:
	...
	void WriteBuffer(uint32 index, uint64 offset, void* data, uint64 size);
	...
	inline VulkanTimingQueryHeap* GetTimingQueryHeap(RenderBackendTimingQueryHeapHandle handle)
	{
		uint32 index = GetRenderBackendHandleRepresentation(handle.GetIndex());
...
static void GetRenderPassDescAndClearValues(VulkanDevice* device, const RenderPassInfo& renderPassInfo, VkImageLayout depthStencilLayout, VulkanRenderPassDesc* outRenderPassDesc, VkClearValue* outClearValues)
{
	...
	for (uint32 index = 0; index < MaxNumSimultaneousColorRenderTargets; index++)
	{
		...
		outClearValues[outRenderPassDesc->numAttachmentDescriptions].color.float32[0] = 0.0f;
		outClearValues[outRenderPassDesc->numAttachmentDescriptions].color.float32[1] = 0.0f;
		outClearValues[outRenderPassDesc->numAttachmentDescriptions].color.float32[2] = 0.0f;
		outClearValues[outRenderPassDesc->numAttachmentDescriptions].color.float32[3] = 0.0f;
	...
	if (renderPassInfo.depthStencilRenderTarget.texture)
	{
		...
		if (bSetExtent)
		{
			...
			outRenderPassDesc->extent.depth = texture->arrayLayers;
			bSetExtent = false;
		}
		else
		{
		...
		outClearValues[outRenderPassDesc->numAttachmentDescriptions].depthStencil.depth = 1.0f;
...
void VulkanDevice::WriteBuffer(uint32 index, uint64 offset, void* data, uint64 size)
{
	MapBuffer(index);	
	VulkanBuffer& buffer = buffers[index];
	memcpy((uint8*)buffer.mappedData + offset, data, size);
	UnmapBuffer(index);
}
...
uint32 VulkanDevice::CreateTexture(const RenderBackendTextureDesc* desc, const void* data, const char* name)
{
	...
	if (HAS_ANY_FLAGS(desc->flags, TextureCreateFlags::DepthStencil))
	{
		...
		VK_CHECK(vkCreateImageView(handle, &imageViewInfo, VULKAN_ALLOCATION_CALLBACKS, &texture.dsv));
	}
...
VulkanFramebuffer* VulkanDevice::FindOrCreateFramebuffer(const RenderPassInfo& renderPassInfo, const VulkanRenderPassDesc& renderPassDesc, VkRenderPass renderPass)
{
	...
	uint64 mipLevelsAndArrayLayers[MaxNumSimultaneousColorRenderTargets];
	...
	if (renderPassDesc.hasDepthStencil)
	{
		...
		framebuffer.attachments[numAttachments] = texture->dsv;
		numAttachments++;
	}
...
class VulkanRenderCompileContext
{
public:
	VulkanRenderCompileContext(VulkanDevice* device, QueueFamily family, VkCommandBuffer commandBuffer)
		: device(device)
		, queueFamily(family)
		...
		, imageBarriers()
		, bufferBarriers() {}
	virtual ~VulkanRenderCompileContext() = default;
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandCopyBuffer& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandCopyTexture& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandBarriers& command)
{
	return true;
}

bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandTransitions& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandBeginTimingQuery& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandEndTimingQuery& command)
{
	const auto& timingQueryHeap = device->GetTimingQueryHeap(command.timingQueryHeap);
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandResolveTimings& command)
{
...
void VulkanRenderCompileContext::ApplyTransitions()
{
...
bool VulkanRenderCompileContext::PrepareForDispatch(RenderBackendShaderHandle shader, const ShaderArguments& shaderArguments)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandDispatch& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandDispatchIndirect& command)
{
	if (!PrepareForDispatch(command.shader, command.shaderArguments))
	{
		return false;
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandUpdateBottomLevelAS& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandUpdateTopLevelAS& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandBeginRenderPass& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandEndRenderPass& command)
{
...
bool VulkanRenderCompileContext::PrepareForDraw(RenderBackendShaderHandle shader, PrimitiveTopology topology, RenderBackendBufferHandle indexBuffer, const ShaderArguments& shaderArguments)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandDraw& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandDrawIndirect& command)
{
	if (!PrepareForDraw(command.shader, command.topology, command.indexBuffer, command.shaderArguments))
	{
		return false;
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandSetViewport& command)
{
...
bool VulkanRenderCompileContext::CompileRenderCommand(const RenderCommandSetScissor& command)
{
...
bool Compile##RenderCommandStructType(VulkanRenderCompileContext* context, void* command)                \
{	                                                                                                     \
 ...
 */
bool (*gCompileRenderCommandFunctions[])(VulkanRenderCompileContext*, void*) = {
...
bool VulkanRenderCompileContext::CompileRenderCommands(const RenderCommandContainer& container)
{
...
static void WriteBuffer(void* instance, RenderBackendBufferHandle handle, uint64 offset, void* data, uint64 size)
{
	VulkanRenderBackend* backend = (VulkanRenderBackend*)instance;
	uint32 deviceMask = handle.GetDeviceMask();
	for (uint32 deviceIndex = 0; deviceIndex < backend->numDevices; deviceIndex++)
	{
		...
		device.WriteBuffer(index, offset, data, size);
	}
}
