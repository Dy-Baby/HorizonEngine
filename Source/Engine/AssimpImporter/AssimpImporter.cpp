	...
	static void ImportAssimpTask(void* data)
	{
		...
		if (aiScene->HasMaterials())
		{
			for (uint32 i = 0; i < aiScene->mNumMaterials; i++)
			{
				...
				Vector3 emission = Vector3(1.0f, 1.0f, 1.0f);
				...
				if (result == aiReturn_SUCCESS)
				{
					material.metallicRoughnessMap = dir + '/' + aiTexPath.C_Str(); s
		...
		if (aiScene->HasMeshes())
		{
			for (uint32 i = 0; i < aiScene->mNumMeshes; i++)
			{
				...
				element.baseVertex = numVertices;
				element.baseIndex = numIndices;
				element.materialIndex = mesh->mMaterialIndex;
				element.numVertices = mesh->mNumVertices;
				element.numIndices = mesh->mNumFaces * 3;
				element.localToWorld = Matrix4x4(1.0f);

				numVertices = element.numVertices;
				numIndices = element.numIndices;
