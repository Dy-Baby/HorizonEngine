...
extern void RenderBackendCreateRenderDevices(RenderBackend* backend, PhysicalDeviceID* physicalDeviceIDs, uint32 numDevices, uint32* outDeviceMasks);
extern void RenderBackendDestroyRenderDevices(RenderBackend* backend);
extern RenderBackendSwapChainHandle RenderBackendCreateSwapChain(RenderBackend* backend, uint32 deviceMask, uint64 windowHandle);
extern void RenderBackendDestroySwapChain(RenderBackend* backend, RenderBackendSwapChainHandle swapChain);
extern void RenderBackendResizeSwapChain(RenderBackend* backend, RenderBackendSwapChainHandle swapChain, uint32* width, uint32* height);
extern bool RenderBackendPresentSwapChain(RenderBackend* backend, RenderBackendSwapChainHandle swapChain);
extern RenderBackendTextureHandle RenderBackendGetActiveSwapChainBuffer(RenderBackend* backend, RenderBackendSwapChainHandle swapChain);
extern RenderBackendBufferHandle RenderBackendCreateBuffer(RenderBackend* backend, uint32 deviceMask, const RenderBackendBufferDesc* desc, const char* name);
extern void RenderBackendResizeBuffer(RenderBackend* backend, RenderBackendBufferHandle buffer, uint64 size);
extern void RenderBackendWriteBuffer(RenderBackend* backend, RenderBackendBufferHandle buffer, uint64 offset, void* data, uint64 size);
extern void RenderBackendDestroyBuffer(RenderBackend* backend, RenderBackendBufferHandle buffer);
extern RenderBackendTextureHandle RenderBackendCreateTexture(RenderBackend* backend, uint32 deviceMask, const RenderBackendTextureDesc* desc, const void* data, const char* name);
extern void RenderBackendDestroyTexture(RenderBackend* backend, RenderBackendTextureHandle texture);
extern RenderBackendTextureSRVHandle RenderBakendCreateTextureSRV(RenderBackend* backend, uint32 deviceMask, const RenderBackendTextureSRVDesc* desc, const char* name);
extern int32 RenderBackendGetTextureSRVDescriptorIndex(RenderBackend* backend, uint32 deviceMask, RenderBackendTextureHandle srv);
extern RenderBackendTextureUAVHandle RenderBackendCreateTextureUAV(RenderBackend* backend, uint32 deviceMask, const RenderBackendTextureUAVDesc* desc, const char* name);
extern int32 RenderBackendGetTextureUAVDescriptorIndex(RenderBackend* backend, uint32 deviceMask, RenderBackendTextureHandle uav);
extern RenderBackendSamplerHandle RenderBackendCreateSampler(RenderBackend* backend, uint32 deviceMask, const RenderBackendSamplerDesc* desc, const char* name);
extern void RenderBackendDestroySampler(RenderBackend* backend, RenderBackendSamplerHandle sampler);
extern RenderBackendShaderHandle RenderBackendCreateShader(RenderBackend* backend, uint32 deviceMask, const RenderBackendShaderDesc* desc, const char* name);
